...
*   **Multi-Document Persistence**: File storage relies on a debounced (500ms) synchronous `localStorage` wrapper scoped in `useWorkspaceManager.ts`. It includes an active LRU file eviction system; if `QuotaExceededError` is triggered, it will quietly delete the oldest untouched user document to preserve the active workspace state.
