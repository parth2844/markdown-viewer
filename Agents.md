...
*   **Multi-Document Persistence**: File storage is persisted in **IndexedDB** using a clean abstraction layer defined in [storageService.ts](file:///d:/Projects/markdown_viewer/src/services/storageService.ts). Legacy files in `localStorage` are automatically migrated on application startup and legacy keys are cleared. Content edits are debounced by 500ms.
*   **Local Folder Access**: Enables direct local directory access using the File System Access API. Folder handles are stored in IndexedDB. Recursive directory traversal and local file I/O operations are managed by [localFolderService.ts](file:///d:/Projects/markdown_viewer/src/services/localFolderService.ts). Collapsible folder trees, file creation, deletion, renames, and permission restoration prompts are handled in [Sidebar.tsx](file:///d:/Projects/markdown_viewer/src/components/Sidebar.tsx).
