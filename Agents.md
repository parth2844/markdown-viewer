...
*   **Component Structure**: Keep all React components inside `src/components/`. Name files `ComponentName.tsx` and prefer utilizing Tailwind utility classes inline whenever possible instead of polluting `.css` files with non-reusable layout logic. **Exception**: For components with complex layouts, absolute positioning layers, hover overlay triggers (like toolbar buttons), or React Portal lightboxes (e.g., `MermaidRenderer`), isolate the layout, padding, transition filters, and z-index declarations into a dedicated stylesheet (e.g., `ComponentName.css`) instead of inline utility classes. This avoids utility class specificity issues, prevents absolute-positioned children from clipping due to container overflows, and simplifies JSX structure.
...
*   **Multi-Document Persistence**: File storage currently relies on a debounced (500ms) synchronous `localStorage` wrapper scoped in `useWorkspaceManager.ts`. We have designed a migration to **IndexedDB** as part of the local folder integration feature (see detailed plan in [local-folder-integration.md](file:///d:/Projects/markdown_viewer/docs/local-folder-integration.md)). The system includes an active LRU file eviction system; if storage quotas are triggered, it will quietly delete the oldest untouched user document to preserve the active workspace state.
*   **Local Folder Integration & IndexedDB Migration**: Detailed specs for connecting local file systems and migrating storage to IndexedDB are mapped out in [local-folder-integration.md](file:///d:/Projects/markdown_viewer/docs/local-folder-integration.md). This feature uses `window.showDirectoryPicker()` and persists folder handles in IndexedDB.
*   **The System Tutorial**: The application's "Getting Started" guide (`default.md`) is loaded as a permanent, reserved `system-tutorial` record. It is mapped securely outside of the user's mutable file array, visually pinned to the top of the Sidebar (`Sidebar.tsx`), and enforces a strict HTML `readOnly` lock upon the Editor when selected.
*   **Auto-Titling**: The workspace hook dynamically parses raw user input for valid Markdown headings (`/^#{1,6}\s/`) to update Sidebar file names in real-time, unless explicitly overwritten by the user via the Sidebar rename input.
...
*   **Mermaid Export Engine & Tainted Canvas Prevention**: Diagram exporting (Copy PNG, Download PNG) is cleanly delegated to the custom hook `useMermaidExport.ts`. To prevent canvas security errors (Tainted Canvases):
    ...
    *   **Lightbox Styling & Full-Bleed Structure**: The modal overlay targets `z-index: 150` to overlay layout navigation bars safely. It implements a full-bleed structure (`p-0` on the outer card with `overflow-hidden`), styling the header and footer segments separately with explicit padded insets (`px-6 py-4 md:px-8`) and distinct background colors (`bg-[var(--bg-secondary)]`) to keep layout elements cleanly separated and offset from card boundaries. Diagram SVGs inside are scaled proportionally (`100%` width/height up to `58vh` maximum height) to optimize readability of massive charts.
    *   **Strip `<pre>` Wrapping**: To prevent nested margins, double margins/borders, and global overrides (like `.markdown-body pre > div`) from interfering with custom preview blocks, intercept and render custom blocks (such as `MermaidRenderer`) without the standard `<pre>` wrapper inside the Markdown components pipeline.
